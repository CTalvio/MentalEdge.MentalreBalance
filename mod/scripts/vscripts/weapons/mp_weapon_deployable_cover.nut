...
void function DeployAmpedWall( entity grenade, vector origin, vector angles )
{
	...
	StatusEffect_AddTimed( ampedWall, eStatusEffect.pass_through_amps_weapon, 1.0, DEPLOYABLE_SHIELD_DURATION_REBALANCE, 0.0 )
	...
	if ( IsValid( owner ) && owner.IsPlayer() )
	{
		...
		foreach ( weapon in offhandWeapons )
		{
			...
			{
				StatusEffect_AddTimed( weapon, eStatusEffect.simple_timer, 1.0, DEPLOYABLE_SHIELD_DURATION_REBALANCE, DEPLOYABLE_SHIELD_DURATION_REBALANCE )
				break
			}
		}
	...
	wait DEPLOYABLE_SHIELD_DURATION_REBALANCE
}
