...
void function GunTipsThread(entity player, int waitTime, int iteration){
    ...
    if (weapon.find("autopistol")){
        messageString2 = "The effective range and damage of the R45 is somewhat reduced"
    ...
    if (weapon.find("weapon_hemlok")){
        messageString1 = "Hemlok is buffed. It can kill in one burst and hipfire accuracy is improved, but ADS recoil is increased."
    ...
    if (weapon.find("weapon_doubletake")){
        messageString1 = "The Doubletake is now able to one-shot if both bolts hit beyond a certain range. Slower reload and rate of fire. Smaller magazine."
    ...
    if (offhand.find("weapon_grenade_electric")){
        messageString4 = "Electric Smoke Grenade does more damage, in a larger radius."
        offhandid1 = "weapon_grenade_electric"
    }
    if (offhand.find("weapon_grenade_emp")){
        messageString4 = "Arc Grenade activates faster."
    ...
    if (offhand.find("ability_shifter")){
        messageString5 = "When phasing, you can now cancel early or extend it to 4 seconds. The time used, affects cooldown."
